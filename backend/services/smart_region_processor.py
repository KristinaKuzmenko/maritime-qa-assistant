        ...
        if table_data and self._is_valid_table_result(table_data):
            ...
            if self.enable_llm_detection:
                ...
                if table_data:
                    ...
                    # Use high detail for better schema detection when table extraction failed
                    verified_type = await self.region_classifier._llm_verify_type(
                        page=fitz_page,
                        region=region,
                        page_num=page_num,
                        use_high_detail=True  # Better quality for re-verification
                    )
...
Output ONLY as specified above."""
        ...
        if not csv_content or len(csv_content) < 5:
            # Check if caption indicates this is a schema/diagram, not a table
            caption_upper = (extracted_caption or '').upper()
            if any(keyword in caption_upper for keyword in ['DIAGRAM', 'FIGURE', 'FIG.', 'DRAWING', 'SCHEMATIC', 'LAYOUT']):
                logger.warning(
                    f"⚠️ LLM returned empty CSV but caption indicates SCHEMA: '{extracted_caption}' - "
                    f"this should be processed as schema, not table"
                )
            else:
                logger.warning(
    ...
    async def _build_table_data_from_csv(
        ...
        try:
            ...
            # Combine all chunks into normalized_text (full table content)
